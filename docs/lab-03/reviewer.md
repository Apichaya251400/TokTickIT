...
| **[#62](https://github.com/Apichaya251400/TokTickIT/pull/62)** — PR 19: Database Schema Evolution, Pure SQL Migration & Idempotent Seed | Approved | The schema, roles, ticket statuses, seed data, comments/notes, ownership fields, and idempotent seed test all look good and are aligned with the Lab 3 requirements. Verified initial password bcrypt hash and explicit attachment preservation. | Merged PR into `lab3-staging` |
| **[#63](https://github.com/Apichaya251400/TokTickIT/pull/63)** — PR 20: Authentication Foundation, Token Revocation, Ownership & Auth API Tests | Pending Review | PR includes authentication foundation, fail-secure JWT_SECRET, server-side token revocation blocklist, requireTicketOwnership and requireAttachmentOwnership middlewares, auth.api.test.ts, and authorization.api.test.ts. | Ready for Peer Review |
